...
public interface IProbabilityFunctionFactory {
    ...
    /**
     * Creates a new SamplePDF instace whose distance is set to 'distance' and whose sampling points
     * are set to samples. As the sampling points are complex values in general, this function sets
     * only the real part of the values. The imaginary part is set to zero.
     * 
     * @param distance
     ...
     * @return
     */
    ISamplePDF createSamplePDFFromDouble(double distance, List<Double> values, boolean isInFrequencyDomain, IUnit unit,
     ...
     * @return
     */
    ISamplePDF createSamplePDFFromComplex(double distance, List<Complex> samples, IUnit unit);
     ...
     * @return
     */
    ISamplePDF createSamplePDFFromComplex(double distance, List<Complex> samples, IUnit unit, IRandomGenerator generator);
     ...
     * @return
     */
    ISamplePDF createSamplePDFFromComplex(double distance, List<Complex> samples, boolean isInFrequencyDomain,
     ...
     *            Sampling points of the approximated PDF.
     * @param isInFrequencyDomain
     *            Indicates whether the values are in frequency domain (true) or time domain
     *            (false).
     * @param unit
     *            TODO
     * @return New instance of SamplePDF with set attributes.
    ...
    /**
     * creates a distribution function, wich represents a dirac impulse. The returned function
     * consists of numOfSamplingPoints equidistant points.
     * 
    ...
    /**
     * Converts a arbitrary PDF to a SamplePDF. So far, only two cases are supported: The input PDF
     * is a SamplePDF, then a reference to the original PDF is returned or the input pdf is a
     * BoxedPDF, then the a new SamplePDF is created from the BoxedPDF.
     * 
     * @param pdf
     *            PDF to transform.
     * @return Either a reference to the input PDF or a new SamplePDF with the same properties as
     *         the input pdf.
     * @throws UnknownPDFTypeException
     *             Thrown in case pdf is neither a BoxedPDF nor a SamplePDF.
     */
    ISamplePDF transformToSamplePDF(IProbabilityDensityFunction pdf) throws UnknownPDFTypeException;

    /**
     * Converts a arbitrary PDF to a SamplePDF. So far, only two cases are supported: The input PDF
     * is a SamplePDF, then a reference to a new Sample PDF with the given distance is returned or
     * the input pdf is a BoxedPDF, then a new SamplePDF is created from the BoxedPDF.
     * 
     * @param pdf
     *            PDF to transform.
     * @param newDistance
     ...
     */
    ISamplePDF transformToSamplePDF(IProbabilityDensityFunction pdf, double newDistance)
     ...
     */
    ISamplePDF transformToSamplePDF(ProbabilityDensityFunction ePDF) throws UnknownPDFTypeException,
     ...
     * 
     * @param samples
     *            List of ContinuousSamples, which defines the upper right corners of the PDF's
     *            boxes. The list may be unsorted. The values of all samples must be greater or
     *            equal to zero.
    ...
    /**
     * Converts a arbitrary PDF to a BoxedPDF. So far, only two cases are supported: The input PDF
     * is a BoxedPDF, then a reference to the original PDF is returned or the input pdf is a
     * SamplePDF, then the a new BoxedPDF is created from the SamplePDF.
     * 
     * @param pdf
     *            PDF to transform.
     * @return Either a reference to the input PDF or a new BoxedPDF with the same properties as the
     *         input pdf.
     * @throws UnknownPDFTypeException
     *             Thrown in case pdf is neither a BoxedPDF nor a SamplePDF.
     * @throws DoubleSampleException
     ...
     * @throws FunctionNotInTimeDomainException
     */
    IBoxedPDF transformToBoxedPDF(IProbabilityDensityFunction pdf) throws UnknownPDFTypeException,
            ProbabilitySumNotOneException, DoubleSampleException, FunctionNotInTimeDomainException;
     ...
     * @throws ProbabilitySumNotOneException
     */
    IBoxedPDF transformToBoxedPDF(ProbabilityDensityFunction ePDF) throws ProbabilitySumNotOneException,
     ...
     * @throws ProbabilitySumNotOneException
     */
    IBoxedPDF transformToBoxedPDF(ProbabilityDensityFunction ePDF, IRandomGenerator randomNumberGenerator)
     ...
     * @throws FunctionNotInTimeDomainException
     */
    BoxedPDF transformToModelBoxedPDF(IProbabilityDensityFunction pdf) throws UnknownPDFTypeException,
            ProbabilitySumNotOneException, DoubleSampleException, FunctionNotInTimeDomainException;

    /**
     * Creates a new ProbabilityMassFunction whose samples are set to 'samples'. If the samples are
     * marked as ordered, they will be sorted and stochastic values such as median and percentile
     * are available.
     * 
     * @param samples
     ...
     * @return
     */
    IProbabilityMassFunction createPMFFromMeasurements(Double[] measurements, double epsilon, IUnit unit,
     ...
     * IBoxedPDF, SamplePDF -> ISamplePDF)
     * 
     * @param ePDF
     *            Object to transform.
     ...
     * @throws ProbabilitySumNotOneException
     */
    IProbabilityDensityFunction transformToPDF(ProbabilityDensityFunction ePDF) throws UnknownPDFTypeException,
            ProbabilitySumNotOneException, DoubleSampleException;
     ...
     * @throws ProbabilitySumNotOneException
     */
    IProbabilityDensityFunction transformToPDF(ProbabilityDensityFunction ePDF, IRandomGenerator randomNumberGenerator)
     ...
     * @throws ProbabilitySumNotOneException
     */
    ProbabilityDensityFunction transformToModelPDF(IProbabilityDensityFunction pdf) throws UnknownPDFTypeException,
            DoubleSampleException, FunctionNotInTimeDomainException;

    /**
     * Set the random generator to use to the passed one. If this method is never called, the
     * {@link DefaultRandomGenerator} is used. This method can be called at any time, however, to
     * get consistent results it should be called before retrieving any functions from this factory.
     * 
