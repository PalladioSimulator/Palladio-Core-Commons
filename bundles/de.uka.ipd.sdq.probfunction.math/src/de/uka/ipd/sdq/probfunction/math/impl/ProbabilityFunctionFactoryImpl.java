...
public class ProbabilityFunctionFactoryImpl implements IProbabilityFunctionFactory {
    ...
    /**
     * can be overwritten by {@link #setRandomGenerator(IRandomGenerator)} to use a predefined
     * random generator in all methods. Otherwise, the default is used.
     */
    ...
    public IBoxedPDF transformToBoxedPDF(ProbabilityDensityFunction epdf) throws ProbabilitySumNotOneException,
            DoubleSampleException {
        return transformToBoxedPDF(epdf, this.randomGenerator);
    }

    public IBoxedPDF transformToBoxedPDF(ProbabilityDensityFunction epdf, IRandomGenerator randomNumberGenerator)
        ...
        if (epdf instanceof BoxedPDF) {
            ...
            for (Object v : ((SamplePDF) epdf).getValues()) {
                samples.add(createContinuousSample(i * ((SamplePDF) epdf).getDistance(), (Double) v));
                i++;
            }
        }
    ...
    public ISamplePDF transformToSamplePDF(ProbabilityDensityFunction epdf) throws UnknownPDFTypeException,
    ...
    public IProbabilityMassFunction transformToPMF(ProbabilityMassFunction epmf, IRandomGenerator randomGenerator) {
        ...
        IProbabilityMassFunction pmf = new ProbabilityMassFunctionImpl(new ArrayList<ISample>(), /*
                                                                                                  * TODO
                                                                                                  * :
                                                                                                  * Unit
                                                                                                  */null,
                hasOrderedDomain, false, randomGenerator);
    ...
    public IBoxedPDF createBoxedPDF(List<IContinuousSample> samples, IRandomGenerator randomGenerator, IUnit unit)
    ...
    public IBoxedPDF createBoxedPDF(List<IContinuousSample> samples, IUnit unit, IRandomGenerator generator)
            throws DoubleSampleException {
    ...
    @Override
    public IProbabilityMassFunction createProbabilityMassFunction(List<ISample> samples, IUnit unit,
            boolean hasOrderedDomain) {
        return new ProbabilityMassFunctionImpl(samples, unit, hasOrderedDomain, false, this.randomGenerator);
    }

    public IProbabilityMassFunction createPMFFromMeasurements(Boolean[] measurements, IUnit unit,
            boolean hasOrderedDomain) {
        ...
        List<ISample> samples = createPMFFromList(times, measurementsList.size());
        return createProbabilityMassFunction(samples, unit, hasOrderedDomain);
    }

    public IProbabilityMassFunction createPMFFromMeasurements(Double[] measurements, double epsilon, IUnit unit,
            boolean hasOrderedDomain) {
    ...
    public IProbabilityMassFunction createPMFFromMeasurements(String[] measurements, IUnit unit,
            boolean hasOrderedDomain) {
    ...
    public IProbabilityMassFunction createPMFFromMeasurements(Integer[] measurements, IUnit unit,
            boolean hasOrderedDomain) {
    ...
    public ISamplePDF createSamplePDFFromComplex(double distance, List<Complex> samples, boolean isInFrequencyDomain,
            IUnit unit) {
    ...
    public ISamplePDF createSamplePDFFromComplex(double distance, List<Complex> samples, IUnit unit,
    ...
    public ISamplePDF createSamplePDFFromComplex(double distance, List<Complex> samples, boolean isInFrequencyDomain,
        ...
        ISamplePDF spdf = new SamplePDFImpl(distance, unit, isInFrequencyDomain, generator);
        spdf.setValues(samples, isInFrequencyDomain);
        return spdf;
    }
    ...
    public ISamplePDF createSamplePDFFromDouble(double distance, List<Double> values, boolean isInFrequencyDomain,
            IUnit unit) {
        ISamplePDF spdf = new SamplePDFImpl(distance, unit, isInFrequencyDomain, this.randomGenerator);
        spdf.setValuesAsDouble(values);
        return spdf;
    ...
    public IBoxedPDF transformToBoxedPDF(IProbabilityDensityFunction pdf) throws UnknownPDFTypeException,
    ...
    @SuppressWarnings("unchecked")
    public BoxedPDF transformToModelBoxedPDF(IProbabilityDensityFunction pdf) throws UnknownPDFTypeException,
            DoubleSampleException, FunctionNotInTimeDomainException {
    ...
    public ProbabilityDensityFunction transformToModelPDF(IProbabilityDensityFunction pdf)
            throws UnknownPDFTypeException, DoubleSampleException, FunctionNotInTimeDomainException {
    ...
    public IProbabilityDensityFunction transformToPDF(ProbabilityDensityFunction ePDF) throws UnknownPDFTypeException,
            ProbabilitySumNotOneException, DoubleSampleException {
        return transformToPDF(ePDF, this.randomGenerator);
    }

    /**
     * TODO: support Pois, Binom, UniInt and UniDouble. They need to be added to the probfunction
     * metamodel and then handled here.
     */
    public IProbabilityDensityFunction transformToPDF(ProbabilityDensityFunction ePDF, IRandomGenerator randomGenerator)
            throws UnknownPDFTypeException, ProbabilitySumNotOneException, DoubleSampleException {
        ...
        if (ePDF instanceof SamplePDF) {
            ...
            pdf = pdfFactory.createExponentialDistribution(((de.uka.ipd.sdq.probfunction.ExponentialDistribution) ePDF)
                    .getRate());
        } else if (ePDF instanceof de.uka.ipd.sdq.probfunction.GammaDistribution) {
            pdf = pdfFactory.createGammaDistribution(((de.uka.ipd.sdq.probfunction.GammaDistribution) ePDF).getAlpha(),
                    ((de.uka.ipd.sdq.probfunction.GammaDistribution) ePDF).getTheta());
        } else if (ePDF instanceof de.uka.ipd.sdq.probfunction.LognormalDistribution) {
            pdf = pdfFactory.createLognormalDistribution(
                    ((de.uka.ipd.sdq.probfunction.LognormalDistribution) ePDF).getMu(),
                    ((de.uka.ipd.sdq.probfunction.LognormalDistribution) ePDF).getSigma());
        } else if (ePDF instanceof de.uka.ipd.sdq.probfunction.NormalDistribution) {
            pdf = pdfFactory.createNormalDistribution(((de.uka.ipd.sdq.probfunction.NormalDistribution) ePDF).getMu(),
                    ((de.uka.ipd.sdq.probfunction.NormalDistribution) ePDF).getSigma());
        } else {
    ...
    public ISamplePDF transformToSamplePDF(IProbabilityDensityFunction pdf, double newDistance)
            throws UnknownPDFTypeException, NegativeDistanceException, FunctionNotInTimeDomainException {
        ISamplePDF resultPDF;
        if (pdf instanceof ISamplePDF) {
    ...
    public IContinuousSample transformToContinuousSample(ContinuousSample eSample) {
        IContinuousSample sample = createContinuousSample(eSample.getValue(), eSample.getProbability());
        return sample;
    }
    ...
    public ISample transformToSample(Sample eSample) {
        ISample sample = createSample(eSample.getValue(), eSample.getProbability());
        return sample;
    }
    ...
    public ISamplePDF transformBoxedToSamplePDF(IBoxedPDF bpdf, double distance) {
        if (bpdf.getSamples().size() == 0)
            return createSamplePDFFromComplex(distance, new ArrayList<Complex>(), bpdf.getUnit());
        ...
        while (currentNewSample < maxValue || index < samples.size()) {
            if (currentNewSample < samples.get(index).getValue()) {
                if (newSamples.size() == 0)
                    ...
                    double dif = index == 0 ? samples.get(0).getValue() : samples.get(index).getValue()
                ...
                if (index < samples.size())
                    newProb += (1 - ((samples.get(index).getValue() - currentNewSample) / (samples.get(index)
                            .getValue() - samples.get(index - 1).getValue()))) * samples.get(index).getProbability();
                newSamples.add(newProb);
                currentNewSample += distance;
                // if (currentNewSample > samples.get(index).getValue())
                // index++;
        ...
        return createSamplePDFFromDouble(distance, newSamples, false, bpdf.getUnit());
    }

    private IBoxedPDF transformSampledToBoxedPDF(ISamplePDF spdf) throws DoubleSampleException,
        ...
        for (int i = 0; i < values.size();) {
            ...
            IContinuousSample s = this.createContinuousSample(distance * i, prob);
            samples.add(s);
        }
    ...
    /**
     * This function creates an approximation histogram of the passed pdf. The number of bins is
     * fixed to 20. If the passed pdf has an infinite support (e.g. ranges from 0 to + infinity like
     * the exponential function), the 95% or 5% percentile is used for the support of the histogram.
     * The value of each bin with the range lowerLimit to upperLimit is the difference of the
     * cumulated probabilities of its limit points: p(bin) = pdf.cdf(upperLimit) -
     * pdf.cdf(lowerLimit).
     * 
     * @param pdf
     *            The continuous probability function to approximate.
     * @return
     * @throws DoubleSampleException
     */
