...
public class SamplePDFImpl extends ProbabilityDensityFunctionImpl implements ISamplePDF {
    ...
    public IProbabilityDensityFunction add(IProbabilityDensityFunction pdf) throws UnknownPDFTypeException,
            FunctionsInDifferenDomainsException, IncompatibleUnitsException {
        return performOperation(Operation.ADD, this, pdf);
    }

    public IProbabilityDensityFunction mult(IProbabilityDensityFunction pdf) throws UnknownPDFTypeException,
            FunctionsInDifferenDomainsException, IncompatibleUnitsException {
    ...
    public IProbabilityDensityFunction scale(double scalar) {
        ...
        return pfFactory.createSamplePDFFromComplex(distance, resultList, this.isInFrequencyDomain(),
                pfFactory.createDefaultUnit());
    }
    ...
    public ISamplePDF getFunctionWithNewDistance(double distance) throws NegativeDistanceException,
    ...
    public double drawSample() {
        double result = 0.0;
        List<Double> intervals = MathTools.computeCumulativeProbabilities(getValuesAsDouble());
    ...
    public String toString() {
        ...
        for (Complex z : values) {
            ...
            result += "(" + MathTools.asString(z.getReal()) + ", " + MathTools.asString(z.getImaginary()) + ")";
        }
        return result;
    }

    private List<Double> getValuesForDistance(double newDistance) throws NegativeDistanceException,
            FunctionNotInTimeDomainException {
        // same distance
        ...
        if (MathTools.equalsDouble(newDistance, distance)) {
            ...
            while (currentIndex < values.size()) {
                if (newPoint < oldPoint) {
                    if (newPoint == newDistance / 2)
                        newValues.add(getProb(currentIndex, newPoint, oldPoint));
                    else
                        newValues.add(getProb(currentIndex, newDistance, distance));

                } else {
                    if (oldPoint == distance / 2) {
                        newValues.add(getLeftProb(oldPoint, currentIndex, newPoint, newDistance, oldPoint)
                                + getRightProb(oldPoint, currentIndex + 1, newPoint));
                    } else
                        newValues.add(getLeftProb(oldPoint, currentIndex, newPoint, newDistance, distance)
                                + getRightProb(oldPoint, currentIndex + 1, newPoint));
                    oldPoint += distance;
                    currentIndex++;
                }
            ...
            while (currentIndex < values.size()) {
                if (oldPoint < newPoint) {
                    ...
                    newValues.add(buffer + getLeftProb(newPoint, currentIndex, oldPoint, distance, distance));
    ...
    private static IProbabilityDensityFunction performOperation(Operation op, IProbabilityDensityFunction pdf1,
            IProbabilityDensityFunction pdf2) throws FunctionsInDifferenDomainsException, UnknownPDFTypeException,
        ...
        IProbabilityDensityFunction p = pfFactory.createSamplePDFFromComplex(distance, resultList, inFrequencyDomain,
                sPDF1.getUnit());
        return p;
    }

    private static List<ISamplePDF> prepareForComputation(IProbabilityDensityFunction pdf1,
            IProbabilityDensityFunction pdf2) throws FunctionsInDifferenDomainsException, UnknownPDFTypeException,
            IncompatibleUnitsException {
        // check preconditions
    ...
    private IProbabilityDensityFunction transformFunction(int flag) {
        ...
        ISamplePDF spdf = pfFactory.createSamplePDFFromComplex(distance, resultList, !this.isInFrequencyDomain(),
                pfFactory.createDefaultUnit());
        return spdf;
    }

    /**
     * Creates two functions with an equal distance, if both functions are in the time domain.
     * Precondition: Functions are in the same domain.
     * 
     * @param pdf1
     * @param pdf2
     * @return
    ...
    public IProbabilityDensityFunction getCumulativeFunction() throws FunctionNotInTimeDomainException {
        ...
        List<Double> cumulativeProbabilities = MathTools.computeCumulativeProbabilities(getValuesAsDouble());
        ISamplePDF spdf = pfFactory.createSamplePDFFromDouble(distance, cumulativeProbabilities, isInFrequencyDomain(),
                this.getUnit(), this.getRandomGenerator());
    ...
    private double compareTo(IProbabilityDensityFunction pdf, CompareOperation op) throws ProbabilityFunctionException {
