...
public class ProbabilityFunctionFactoryTest {
    ...
    public void boxedToSamplePDF() {
        ...
        Collections.addAll(samples, pfFactory.createContinuousSample(0.9, 0.3),
                pfFactory.createContinuousSample(1.5, 0.4), pfFactory.createContinuousSample(1.8, 0.2),
                pfFactory.createContinuousSample(2.4, 0.1));
        try {
        ...
        try {
            ...
            assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(0) - 0.3 * 0.15 / 0.9) < err);
            ...
            assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(3) - ((0.3 * 0.15 / 0.9) + (0.4 * 0.15 / 0.6))) < err);
            assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(4) - 0.4 * 0.3 / 0.6) < err);
            assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(5) - ((0.4 * 0.15 / 0.6) + (0.2 * 0.15 / 0.3))) < err);
            assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(6) - ((0.2 * 0.15 / 0.3) + (0.1 * 0.15 / 0.6))) < err);
            assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(7) - (0.1 * 0.3 / 0.6)) < err);
            assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(7) - (0.1 * 0.3 / 0.6)) < err);
            assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(8) - (0.1 * 0.15 / 0.6)) < err);
        } catch (UnknownPDFTypeException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }

        samplePDF = ((ProbabilityFunctionFactoryImpl) pfFactory).transformBoxedToSamplePDF(boxed, 0.5);
        assertTrue(Math.abs(samplePDF.getDistance() - 0.5) < err);
        assertEquals(samplePDF.getValuesAsDouble().size(), 6);

        assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(0) - 0.3 * 0.25 / 0.9) < err);
        assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(1) - 0.3 * 0.5 / 0.9) < err);
        assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(2) - ((0.3 * 0.15 / 0.9) + (0.4 * 0.35 / 0.6))) < err);
        assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(3) - ((0.4 * 0.25 / 0.6) + (0.2 * 0.25 / 0.3))) < err);
        assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(4) - ((0.2 * 0.05 / 0.3) + (0.1 * 0.45 / 0.6))) < err);
        assertTrue(Math.abs(samplePDF.getValuesAsDouble().get(5) - (0.1 * 0.15 / 0.6)) < err);
    }

    /**
     * Transforming IProbabilityMassFunction to ProbabilityMassFunction should leave samples, unit,
     * and ordered domain intact.
     */
    @Test
    public void iPMFToPMF() {
        IProbabilityMassFunction iProbFunc = pfFactory.createProbabilityMassFunction(getTestSamples(), getTestUnit(),
                true);

        // this is the method to be tested:
        ProbabilityMassFunction probFunc = pfFactory.transformToModelPMF(iProbFunc);
    ...
    public void ePMFToIPMF() {
        ProbabilityMassFunction epmf = epfFactory.createProbabilityMassFunction();
    ...
    public void createISamplePDF() {
        List<Double> samples = new ArrayList<Double>();
        Collections.addAll(samples, 0.1, 0.3, 0.2, 0.2, 0.2);
        ISamplePDF spdf = pfFactory.createSamplePDFFromDouble(0.1, samples, false, getTestUnit());
    ...
    public void createPMFFromMeasurements() {
        Double[] measurements = {
                0.123, 0.340, 0.124, 0.343, 1.934, 0.345, 1.935, 1.940, 1.945, 2.134
        };
        IProbabilityMassFunction pmf = pfFactory.createPMFFromMeasurements(measurements, 0.01, getTestUnit(), true);
        ...
        String[] m3 = {
                "test1", "test2", "1test", "test1", "1test", "test3", "test3", "test1", "test2", "test2"
        };
    ...
    @Test
    public void transformContinuousToBoxedPDF() throws UnknownPDFTypeException, ProbabilitySumNotOneException,
            DoubleSampleException, FunctionNotInTimeDomainException, NegativeDistanceException, UnitNotSetException,
    ...
    private void transformAndCheck(IProbabilityDensityFunction func) throws UnknownPDFTypeException,
            ...
            NegativeDistanceException, UnitNotSetException, UnitNameNotSetException, InvalidSampleValueException {
    ...
    private List<ISample> getTestSamples() {
        Object[] testSamples = {
                1, 0.1, 2, 0.3, 3, 0.5, 4, 0.1
        };
        List<ISample> sList = new ArrayList<ISample>();
        for (int i = 0; i < testSamples.length; i += 2) {
            ISample s = pfFactory.createSample(testSamples[i], (Double) testSamples[i + 1]);
            sList.add(s);
        }
        return sList;
    }
    ...
    private List<IContinuousSample> getTestContinuousSamples() {
        Double[] testSamples = {
                1.0, 0.1, 2.0, 0.3, 3.0, 0.5, 4.0, 0.1
        };
        List<IContinuousSample> sList = new ArrayList<IContinuousSample>();
        for (int i = 0; i < testSamples.length; i += 2) {
            IContinuousSample s = pfFactory.createContinuousSample(testSamples[i], (Double) testSamples[i + 1]);
            sList.add(s);
        }
        return sList;
    }
    ...
    private void initTestEContinuousSamples(EList esList) {
        Double[] testSamples = {
                2.1, 0.1, 3.5, 0.3, 6.2, 0.5, 6.7, 0.1
        };
        for (int i = 0; i < testSamples.length; i += 2) {
